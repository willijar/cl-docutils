...
(defmethod visit-node ((writer html-writer) (document document))
  (let ((*section-level* 0))
    (with-part(head)
      (when (setting :stylesheet document)
        (part-append
         (if (setting :link-stylesheet document)
             (format nil
                     "<link rel=\"stylesheet\" href=\"~A\" type=\"text/css\" />~%"
                     (setting :stylesheet document))
             (with-output-to-string(os)
               (write-line "<style type=\"text/css\">" os)
               (with-open-file(is (setting :stylesheet document)
  ...
  (let* ((atts nil)
    ...
    (setf (getf atts :src)
          (if (and (pathnamep uri) media-server)
              ...
              uri))
    (remf atts :uri)
    (if (setting :image-sizes (document node))
